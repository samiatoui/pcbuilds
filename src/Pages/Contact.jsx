function Contact() {
  return (
    <div className="pagetitle">
      <h1>Contact Us</h1>
      <div style={{ marginTop: "50px" }}>
        <h3>For all inquiries, please call us at (204) 997-7121</h3>
        <h3> You may also email us at primetechwpg@gmail.com</h3>
        <h3>
          Follow us on{" "}
          <a className="contact-a" href="https://instagram.com/primetech_wpg">
            Instagram
          </a>
          .
        </h3>
      </div>
      <div className="disclaimer">
        <p>We do not have a storefront.</p>
        <p>
          Repairs can be done through in-home services or by dropping
          off/mailing in your device.
        </p>
        <p>
          We are committed to providing top-quality service and ensuring your
          complete satisfaction.
        </p>
      </div>
    </div>
  );
}

export default Contact;
